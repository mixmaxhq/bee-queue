...
Queue.prototype._scanForJobs = function (cursor, set, cb) {
  ...
  // For an MVP, however, we're aiming to replace our usage of Arena for Bull,
  // and furthermore Bull 1.x calls SMEMBERS -- which is more blocking than SSCAN.

  this.client.sscan(this.toKey('failed'), cursor, 'COUNT', this.settings.redisScanCount, (err, results) => {
...
Queue.prototype._getJobs = function (queueType, start, end, cb) {
  ...
  if (this.paused) {
    ...
    promise = this._ready
      ...
      .then((ids) => {
        // If ids is a Set (i.e. returned from Queue._scanForJobs), convert it to an Array
        ids = Array.from(ids);
...
Queue.prototype.getJobs = function (type, start, end, cb) {
  return this._getJobs(type, start, end, cb);
};
...
Queue.prototype.runJob = function (job, cb) {
  if (this.settings.verbose) {
    this.emit('runJob', {id: job.id, timestamp: job.options.timestamp});
  }
  ...
  if (this.settings.verbose) {
    this.emit('callingHandler', {id: job.id, timestamp: job.options.timestamp});
  }
...
Queue.prototype._finishJob = function (err, data, job, cb) {
  ...
  if (err) {
    job.options.stacktraces.unshift(err ? err.stack || err.message || err : null);
...
Queue.prototype.process = function (concurrency, handler) {
  ...
  const jobTick = () => {
    ...
    this.getNextJob((getErr, job) => {
      ...
      if (this.settings.verbose) {
        this.emit('gotJob', {id: job.id, timestamp: job.options.timestamp});
      }
