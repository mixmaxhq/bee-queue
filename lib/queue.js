...
Queue.prototype._scanForJobs = function(cursor, set, cb) {
  // Ideally, we would be able to paginate redis sets like we do sorted ranges.
  // For an MVP, however, we're aiming to replace our usage of Arena for Bull 1.x
  // and Bull 1.x calls SMEMBERS -- which is more blocking than SSCAN.
  ...
  this.client.sscan(
    ...
    (err, results) => {
      ...
      this._scanForJobs(cb, nextCursor, set);
    }
  );
}

// TODO(randall): Implement (start, end) parameters
Queue.prototype._getJobs = function (queueType, start, end, cb) {
  let promise;
    ...
    this._ready
      ...
      .then(() => {
        switch (queueType) {
          case 'failed':
          case 'succeeded':
            this._scanForJobs('0', new Set(), idsPromise.defer());
            break;
          case 'waiting':
          case 'active':
            this.client.lrange(this.toKey(queueType), start, end, idsPromise.defer());
            break;
          case 'delayed':
            this.client.zrange(this.toKey(queueType), start, end, idsPromise.defer());
            break;
        }
      });

    promise = idsPromise.then((ids) => {
      return Promise.all([...ids].map((id) => this.getJob(id)));
    });
  }
...
Queue.prototype.getWaiting = function (start, end, cb) {
  return this._getJobs('waiting', start, end, cb);
}

Queue.prototype.getActive = function (start, end, cb) {
  return this._getJobs('active', start, end, cb);
}

Queue.prototype.getSucceeded = function (cb) {
  return this._getJobs('succeeded', null, null, cb);
}

Queue.prototype.getFailed = function (cb) {
  return this._getJobs('failed', null, null, cb);
}

Queue.prototype.getDelayed = function (start, end, cb) {
  return this._getJobs('delayed', start, end, cb);
}
...
Queue.prototype._finishJob = function (err, data, job, cb) {
  ...
  if (err) {
    job.options.attemptsMade += 1;
    job.options.stacktraces.unshift(err.stack);
