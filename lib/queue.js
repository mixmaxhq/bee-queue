...
Queue.prototype.getNextJob = function (cb) {
  const promise = new Promise((resolve, reject) => {
    ...
    this.bclient.brpoplpush(this.toKey('waiting'), this.toKey('active'), 0, (err, jobId) => {
      ...
      resolve(Job.fromId(this, parseInt(jobId, 10)));
    });
  });
...
Queue.prototype.runJob = function (job, cb) {
  ...
  promise = promise.then((data) => handleOutcome(null, data), handleOutcome);
  this.activeJobs.add(promise);

  // TODO: will this prevent an unhandled rejection?
  promise.catch(() => {}).then(() => this.activeJobs.delete(promise));
...
Queue.prototype._finishJob = function (err, data, job, cb) {
  ...
  if (cb) promise.then(() => cb(status, result), cb);
