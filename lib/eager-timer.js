...
class EagerTimer extends Emitter {
  ...
  schedule(time) {
    ...
    // Only overwrite the existing timer if the existing timer is later than the given time.
  ...
  _trigger() {
    ...
    /* istanbul ignore if */
    if (remaining > 0) {
      // It's possible (it caused bugs) to have the timeout trigger before the destination time.
